...
Item {
    ...
    function paintView(ctx) {
        ...
        // Note: Canvas keeps previous content, so needs clearing
        ctx.reset();
    ...
    function drawGraphCircles(ctx, x, y, w, h, items, t)
    {
        ...
        for (let i = 0; i < items; i++) {
            ctx.beginPath();
            // Note: Using standard arc() instead of circle().
    ...
    function drawGraphLine(ctx, x, y, w, h, items, t)
    {
        ...
        for (i = 0; i < items; i++) {
            // Note: Using standard arc() instead of circle().
            // Use moveTo() to avoid  straight line from the last point
            // in the subpath to the start point of the arc.
            ctx.moveTo(sx[i] + dotSize * 0.8, sy[i]);
            ctx.arc(sx[i], sy[i], dotSize * 0.8, 0, 2 * Math.PI);
        }
    ...
    function drawFlower(ctx, x, y, w, h, t)
    {
        ...
        ctx.fillStyle = "#ffffff";
        ctx.beginPath();
        ctx.arc(cx, cy, 0.1 * w, 0, 2 * Math.PI);
        ctx.fill();
        ctx.resetTransform();
    }
}
